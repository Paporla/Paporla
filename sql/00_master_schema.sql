...
-- 18.1 Prevenir escalación de privilegios (cambio de rol sin ser admin)
...
DROP TRIGGER IF EXISTS trigger_prevent_role_escalation ON public.user_profiles;
CREATE TRIGGER trigger_prevent_role_escalation
  BEFORE UPDATE ON public.user_profiles
  FOR EACH ROW
  EXECUTE FUNCTION public.prevent_role_escalation();

-- 18.2 Eliminar UPDATE directo en reservations por usuario
-- Los cambios de estado deben pasar por RPC (cancel_reservation, validate_pickup, etc.)
-- que son SECURITY DEFINER y no necesitan esta política
DROP POLICY IF EXISTS "reservations_client_update" ON public.reservations;

-- ========================================================
-- 19. TABLA: rate_limits (para rate limiting serverless)
-- ========================================================
...
-- ========================================================
-- 20. VISTA: available_packs (listado público de packs disponibles)
-- ========================================================

CREATE OR REPLACE VIEW public.available_packs AS
SELECT 
  p.id, p.shop_id, p.title, p.description,
  p.price_cents, p.original_price_cents, p.discount_percentage,
  p.remaining_stock, p.total_stock,
  p.pickup_date, p.pickup_start_time, p.pickup_end_time,
  p.image_url, p.created_at, p.is_active, p.status,
  p.deleted_at, p.updated_by,
  s.name AS shop_name,
  s.address AS shop_address,
  s.city AS shop_city,
  s.rating AS shop_rating,
...
GRANT SELECT ON public.available_packs TO anon, authenticated;

-- También damos acceso a la vista para service_role
GRANT ALL ON public.available_packs TO service_role;
