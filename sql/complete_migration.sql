...
BEGIN
  ...
  VALUES (
    ...
    'Se ha realizado una reserva para el pack ' || NEW.pack_id || ' con codigo ' || COALESCE(NEW.pickup_code, 'N/A'),
...
BEGIN
  ...
  VALUES (
    ...
    'Se ha creado un nuevo pack: ' || NEW.title,
...
BEGIN
  ...
  VALUES (
    ...
    'Se ha registrado un nuevo comercio: ' || NEW.name,
...
-- Log: Reserva cancelada o recogida
...
BEGIN
  IF OLD.status != NEW.status THEN
    ...
    VALUES (
      ...
      END,
      'Reserva ' || NEW.status,
      'La reserva ' || COALESCE(NEW.pickup_code, 'N/A') || ' ha cambiado a estado: ' || NEW.status,
...
-- Triggers de activity_logs (ver archivo activity_logs_triggers.sql)
