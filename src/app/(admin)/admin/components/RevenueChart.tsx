  ...
  if (active && payload && payload.length) {
    return (
      <div className="dark:bg-black/90 bg-white/90 backdrop-blur-sm dark:border-white/10 border-gray-200 rounded-xl p-3 shadow-xl">
        ...
        <p className="text-xs dark:text-gray-500 text-gray-400 mt-1">Reservas: {payload[0].payload.total_reservations}</p>
      </div>
    );
  }
  return null;
  ...
  if (data.length === 0) {
    return (
      <div className="dark:bg-black/40 bg-gray-50 backdrop-blur-sm dark:border-white/10 border-gray-200 rounded-2xl p-6 text-center">
        ...
        <p className="text-xs dark:text-gray-600 text-gray-500 mt-1">Los datos se mostraran cuando haya reservas completadas</p>
      </div>
    );
  }
  ...
  return (
    <div className="dark:bg-black/40 bg-gray-50 backdrop-blur-sm dark:border-white/10 border-gray-200 rounded-2xl p-6">
      ...
      <div className="flex items-baseline gap-3 mb-6">
        <span className="text-3xl font-black dark:text-white text-gray-900">${(totalRevenue / 100).toLocaleString()}</span>
        {growth !== 0 && (
