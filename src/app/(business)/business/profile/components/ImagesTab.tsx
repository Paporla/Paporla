...
export default function ImagesTab({ formData, updateForm }: ImagesTabProps) {
  return (
    <div className="dark:bg-black/40 bg-white backdrop-blur-sm dark:border-white/10 border-gray-200 rounded-2xl p-6 lg:p-8 space-y-8">
      ...
      <ImageUpload
        bucket="shop-images"
        path={`shops/${Date.now()}/logo`}
        existingImage={formData.logoUrl}
        onUploadComplete={(url) => updateForm('logoUrl', url)}
        onError={() => {}}
        label="Logo del comercio"
      />
      <ImageUpload
        bucket="shop-images"
        path={`shops/${Date.now()}/cover`}
        existingImage={formData.coverUrl}
        onUploadComplete={(url) => updateForm('coverUrl', url)}
        onError={() => {}}
