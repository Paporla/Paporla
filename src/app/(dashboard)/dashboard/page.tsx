...
export default function UserDashboardPage() {
  ...
  const loadDashboardData = async () => {
    ...
    const formatted = await Promise.all(
      reservationsData.map(async (reservation: any) => {
        const [packResult, shopResult] = await Promise.all([
          supabase.from('packs').select('*').eq('id', reservation.pack_id).maybeSingle(),
          supabase.from('shops').select('*').eq('id', reservation.shop_id).maybeSingle(),
        ]);
        return { ...reservation, pack: packResult.data, shop: shopResult.data };
      })
    );

    const validReservations = formatted.filter(r => r.pack && r.shop);
    const active = validReservations.filter(r => ['confirmed', 'pending'].includes(r.status));
    const completed = validReservations.filter(r => r.status === 'picked_up');
    ...
    const recentActivities = validReservations.slice(0, 5).map(r => ({
      id: r.id,
        ...
        {nextReservation ? (
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-semibold text-white">Próxima recogida</h2>
            </div>
