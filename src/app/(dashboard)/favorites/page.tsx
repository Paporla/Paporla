...
export default function FavoritesPage() {
  ...
  const handleRemove = async (shopId: string) => {
    ...
    if (error) setError('Error al eliminar de favoritos');
    else {
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {favorites.map((fav, idx) => (
          <motion.div key={fav.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }}>
            <Card glass className="p-4 group hover:border-primary/30 transition-all">
              <div className="flex gap-4">
                <div className="w-16 h-16 rounded-xl bg-primary/10 flex items-center justify-center flex-shrink-0">
                  {fav.shop.logo_url ? (
                    <img src={fav.shop.logo_url} alt={fav.shop.name} className="w-full h-full object-cover rounded-xl" />
                  ) : (
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <div className="flex items-center gap-1 mt-0.5">
                        ...
                        {fav.shop.verified && <span className="text-[10px] text-green-400 bg-green-500/10 px-1.5 py-0.5 rounded-full">Verificado</span>}
                      </div>
                      {fav.shop.address && <p className="text-xs text-gray-500 flex items-center gap-1 mt-1"><MapPin className="w-3 h-3" /><span className="truncate">{fav.shop.address}</span></p>}
                    </div>
                    <Button variant="danger" size="sm" onClick={() => handleRemove(fav.shop_id)} loading={removing === fav.shop_id} className="p-2"><Heart className="w-4 h-4" /></Button>
                  </div>
                  <div className="flex gap-2 mt-3">
                    <Button variant="outline" size="sm" onClick={() => router.push(`/shops/${fav.shop.id}`)} className="flex-1">Ver comercio</Button>
                    <Button size="sm" onClick={() => router.push(`/packs?shop=${fav.shop.id}`)} className="flex-1">Ver packs</Button>
                  </div>
                </div>
              </div>
            </Card>
