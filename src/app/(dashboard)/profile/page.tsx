...
export default function ProfilePage() {
  ...
  const loadUserStats = async () => {
    ...
    let level = 'Aprendiz';
    if (points >= 500) level = 'Rescatador Élite';
    ...
    >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card glass className="p-4 text-center">
          ...
          <p className="text-2xl font-bold text-white">{stats.packsRescued}</p>
          <p className="text-xs text-gray-500">Packs rescatados</p>
        </Card>
        <Card glass className="p-4 text-center">
          <DollarSign className="w-6 h-6 text-green-400 mx-auto mb-2" />
          <p className="text-2xl font-bold text-white">${stats.moneySaved.toFixed(0)}</p>
          <p className="text-xs text-gray-500">Ahorrado</p>
        </Card>
        <Card glass className="p-4 text-center">
          <Leaf className="w-6 h-6 text-green-400 mx-auto mb-2" />
          <p className="text-2xl font-bold text-white">{stats.co2Avoided}</p>
      ...
      <Card glass className="p-5">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-white">Nivel {stats.level}</h3>
          </div>
          <span className="text-xs text-gray-500">{stats.points} pts</span>
        </div>
        ...
        <p className="text-xs text-gray-500 mt-2">
          {stats.points >= 500 
            ...
            : `Faltan ${500 - stats.points} puntos para llegar a Rescatador Élite`}
        </p>
      </Card>
        ...
        <Card glass className="p-6">
          <div className="flex items-center justify-between p-4 bg-black/40 rounded-xl">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">Salir de tu cuenta actual</p>
            </div>
