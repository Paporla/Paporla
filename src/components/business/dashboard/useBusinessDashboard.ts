...
export function useBusinessDashboard() {
  ...
  const [stats, setStats] = useState<DashboardStats>({
    totalPacks: 0, activePacks: 0, totalReservations: 0, pendingReservations: 0, totalRevenue: 0,
  })
  ...
  const loadBusinessData = async () => {
    ...
    const { data: shopData } = await supabase
      .from('shops').select('id, name, verified, logo_url').eq('owner_id', user.id).maybeSingle()
    ...
    const { data: packsData } = await supabase
      .from('packs').select('id, title, remaining_stock, is_active, price_cents')
    ...
    const { data: rawReservations } = await supabase
      .from('reservations')
      .select(`id,user_id,quantity,total_price_cents,status,created_at,pack_id,pack:packs(title)`)
    ...
    const enriched = await Promise.all(
      rawList.map(async (r: any) => {
        const { data: userData } = await supabase
          .from('user_profiles').select('name, email').eq('id', r.user_id).maybeSingle()
        return { ...r, user: userData || { name: 'Usuario', email: '' } }
      })
    )
    ...
    setStats({
      ...
      pendingReservations: reservations.filter(r => ['confirmed', 'pending'].includes(r.status)).length,
