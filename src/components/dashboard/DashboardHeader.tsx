  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl md:text-4xl font-bold text-gradient mb-2">Hola, {user?.name || 'Usuario'}!</h1>
      ...
      {level.nextThreshold && (
        <div className="dark:bg-black/40 bg-gray-100 rounded-xl p-3">
          <div className="flex items-center justify-between text-xs dark:text-gray-400 text-gray-600 mb-1">
            <span>
              Progreso a{' '}
              {level.name === 'Rescatador'
                ? 'Rescatador'
                : level.nextThreshold === 50
                  ? 'Rescatador Elite'
                  : 'Rescatador Pro'}
            </span>
            <span>
              {points}/{level.nextThreshold} pts
            </span>
          </div>
