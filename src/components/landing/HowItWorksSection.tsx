...
export default function HowItWorksSection() {
  return (
    <motion.div
      ...
      className="h-full"
    >
      <div className="h-full island-dark backdrop-blur-sm rounded-2xl p-8 border border-white/10 dark:border-primary/20 group hover:border-white/25 dark:hover:border-primary/40 transition-all duration-300">
            ...
            {steps.map((step, idx) => (
              ...
              >
                ...
                <div className="relative z-10">
                  <div className="relative w-10 h-10 rounded-full bg-[#0a0a0f] shadow-inner border border-primary/30 flex items-center justify-center group-hover/item:scale-110 group-hover/item:shadow-lg group-hover/item:shadow-primary/30 transition-all duration-300">
                    <div className="absolute inset-0 rounded-full bg-gradient-to-br from-primary/30 to-primary/10" />
                    <span className="relative z-10 island-accent font-bold text-base">{idx + 1}</span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 w-60">
                  <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-primary/[0.12] to-primary/[0.04] flex items-center justify-center flex-shrink-0 group-hover/item:scale-110 group-hover/item:-rotate-3 group-hover/item:shadow-lg transition-all duration-300">
                    <step.icon className="w-7 h-7 island-accent" />
                  </div>
                  <div>
                    <p className="font-bold text-white text-base group-hover/item:text-[color:var(--island-accent)] transition-colors duration-300">
                      {step.title}
                    </p>
                    <p className="text-sm island-muted mt-1 leading-relaxed">{step.description}</p>
                  </div>
                </div>
              </motion.div>
            ))}
