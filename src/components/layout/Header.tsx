...
export default function Header() {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-[60] transition-all duration-300 ${isScrolled ? 'bg-gradient-to-b from-primary/[0.18] to-black/80 backdrop-blur-xl border-b border-primary/10' : 'bg-transparent'}`}
    >
          ...
          >
            <div className="px-4 py-4 flex flex-col gap-3">
              ...
              {!user && (
                <Link
                  href="/register"
                  className="px-4 py-2 bg-primary text-on-primary rounded-lg text-center font-medium"
                  onClick={() => setIsMenuOpen(false)}
                >
                  Registrarse
                </Link>
