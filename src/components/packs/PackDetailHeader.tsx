...
export default function PackDetailHeader({
  ...
  return (
    <div className="sticky top-0 z-10 flex items-center justify-between p-4 bg-dark/80 backdrop-blur-xl">
      <motion.button
        whileHover={{ scale: 1.05 }}
        whileTap={{ scale: 0.95 }}
        onClick={onBack}
        className="p-2 bg-dark-muted rounded-full hover:bg-dark-hover transition-colors"
      >
      ...
      {isLowStock && (
        <div className="absolute top-20 right-4 bg-red-500/90 backdrop-blur-sm text-white text-[10px] font-bold px-2.5 py-1 rounded-full flex items-center gap-1 animate-pulse">
          ...
          {remainingStock === 1 ? '¡Último!' : `${remainingStock} disponibles`}
        </div>
      )}

      {/* Precio flotante */}
      <div className="absolute bottom-4 left-4 right-4 bg-dark-card/90 backdrop-blur-sm rounded-xl p-3 flex items-center justify-between">
        <div className="flex items-baseline gap-2">
          <span className="text-2xl font-black text-primary">{formatPrice(priceCents)}</span>
          {originalPriceCents && (
            <span className="text-sm text-gray-600 line-through">{formatPrice(originalPriceCents)}</span>
          )}
        </div>
