...
interface Filters {
  search: string; minPrice: number; maxPrice: number; showAvailableOnly: boolean
...
const defaultFilters: Filters = { search: '', minPrice: 0, maxPrice: 100000, showAvailableOnly: false, city: '', location: null, radiusKm: 10 }

export default function PackFiltersAdvanced({ onFilterChange, initialFilters = {}, cities = ['Caracas', 'Barquisimeto', 'Maracaibo', 'Valencia', 'Maracay'] }: Props) {
  const [isOpen, setIsOpen] = useState(false)
  const [filters, setFilters] = useState<Filters>({ ...defaultFilters, ...initialFilters })

  const activeCount = [filters.minPrice > 0, filters.maxPrice < 100000, filters.showAvailableOnly, !!filters.city, !!filters.location].filter(Boolean).length

  const notify = useCallback((f: Filters) => { onFilterChange(f) }, [onFilterChange])
  ...
  const clearAll = () => { setFilters(defaultFilters); notify(defaultFilters) }
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}>
            <div className="glass-card rounded-2xl p-6 backdrop-blur-sm space-y-6">
              <div className="flex items-center justify-between">
                ...
                <button onClick={clearAll} className="text-sm text-primary hover:text-primary/80 flex items-center gap-1"><X className="w-4 h-4" /> Limpiar todos</button>
              </div>
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2">
                  <GeolocationFilter onLocationChange={(coords, radius) => { update('location', coords); update('radiusKm', radius); if (coords) update('city', '') }} defaultRadius={filters.radiusKm} />
                </div>
