...
import type { PublicPack } from './PackCardPublic'
...
export function usePublicPacks() {
  ...
  const [filters, setFilters] = useState<Filters>({
    search: '', minPrice: 0, maxPrice: 100000, showAvailableOnly: false, city: '',
  })
  ...
  const loadPacks = async () => {
    ...
    const { data, error: err } = await supabase
      .from('available_packs').select('*').order('created_at', { ascending: false })

    if (err) setError(err.message)
    else setAllPacks(data || [])
    setLoading(false)
  }
