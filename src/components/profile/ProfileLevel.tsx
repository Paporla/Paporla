...
export default function ProfileLevel({ level, points, nextLevelPoints, nextLevelName }: ProfileLevelProps) {
  ...
  return (
    <motion.div
      ...
      className="dark:bg-dark-card bg-white dark:border-dark-border border-gray-200 rounded-2xl p-5"
    >
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-xs dark:text-gray-500 text-gray-400">Nivel actual</p>
            <p className="font-bold dark:text-white text-gray-900">{level}</p>
          </div>
        </div>
      ...
      <div className="space-y-2">
        <div className="flex justify-between text-xs dark:text-gray-500 text-gray-400">
          <span>Progreso a {nextLevelName}</span>
          <span>{Math.round(progress)}%</span>
        </div>
        <div className="h-2 dark:bg-dark-muted bg-gray-200 rounded-full overflow-hidden">
          <motion.div
            initial={{ width: 0 }}
