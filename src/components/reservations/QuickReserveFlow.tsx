...
import { CheckCircle, Clock, MapPin, ShoppingBag, X, Navigation } from 'lucide-react'
...
export default function QuickReserveFlow({ pack, onClose, onSuccess }: QuickReserveFlowProps) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      const result = await createReservation({ packId: pack.id, quantity: 1, paymentMethod: 'demo' })
          ...
          {step === 'review' && (
            ...
            >
              <div className="relative h-48 bg-gradient-to-br from-primary/20 to-secondary/20">
                {pack.image_url ? (
                  <Image src={pack.image_url} alt={pack.title} fill className="object-cover" sizes="400px" />
                ) : null}
                <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent" />
                <button
                  onClick={onClose}
                  className="absolute top-3 right-3 p-2 rounded-full dark:bg-black/40 bg-black/20 dark:text-white text-gray-900 hover:dark:bg-black/60 hover:bg-black/30"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
              <div className="p-5 space-y-4">
                ...
                {pack.pickup_date && (
                  <div className="flex items-center gap-2 text-sm dark:text-gray-400 text-gray-600">
                    ...
                    <span>
                      {formatDate(pack.pickup_date)} {pack.pickup_start_time?.slice(0, 5)} -{' '}
                      {pack.pickup_end_time?.slice(0, 5)}
                    </span>
                  </div>
                )}
                {pack.shop.address && (
            ...
            >
              <div className="p-6 text-center space-y-4">
                ...
                <div className="bg-gradient-to-r from-primary/10 to-primary/5 border border-primary/30 rounded-xl p-5">
                  ...
                  <p className="text-4xl font-bold text-primary tracking-[0.3em] font-mono py-2">
                    {confirmedData.pickup_code}
                  </p>
