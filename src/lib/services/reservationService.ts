...
export async function updateReservation(userId: string, body: { id: string; status: string; cancel_reason?: string }) {
  ...
  // Usar admin client para el UPDATE: la autorización ya se validó arriba
  // y no hay política RLS para UPDATE directo (cambios de estado van por RPC)
  const adminClient = getSupabaseAdmin()
  const { data: updated, error } = await adminClient.from('reservations').update(updates).eq('id', id).select().single()
  if (error) return { error: error.message, status: 500 }
